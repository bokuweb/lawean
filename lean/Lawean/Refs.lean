...
def kanji (n : Nat) : String :=
  ...
  if n = 0 then "〇"
  else if n < 10 then d[n]!
  else if n < 20 then "十" ++ d[n % 10]!
...
/-- **描画は参照の番号にしか依存しない**: 各参照の (番号, 距離) が同じなら描画は同じ -/
theorem renderBody_congr (r r' : Revision) (src : NodeId) :
    ∀ body : Body, (∀ t f, .ref t f ∈ body → refNums r src t = refNums r' src t) →
  ...
  | .ref t f :: ps, h => by
    simp only [renderBody, renderPiece]
    rw [h t f (List.mem_cons_self _ _), renderBody_congr r r' src ps (fun t' f' ht => h t' f' (List.mem_cons_of_mem _ ht))]

/-- **ハネの完全性**（対偶）: 本文が同じなのに描画が違えば、どれかの参照の番号か距離が動いている -/
theorem hane_complete (r r' : Revision) (src : NodeId) (body : Body)
    (h : renderBody r src body ≠ renderBody r' src body) :
    ∃ t f, .ref t f ∈ body ∧ refNums r src t ≠ refNums r' src t :=
...
theorem haneFixes_sound (r r' : Revision) (bodies : Bodies) (id : NodeId) (b a : String)
    ...
    ∃ body, (id, body) ∈ bodies ∧ ∃ t f, .ref t f ∈ body ∧ refNums r id t ≠ refNums r' id t := by
