...
import Lawean.Data.Rev_414AC0000000078_draft_r2_62
import Lawean.Data.Rev_414AC0000000078_20220401
import Lawean.Data.Unit_derived_414AC0000000078_20220401
import Lawean.Data.Bodies_502AC0000000062_art2
...
/-- 令2-62 第2条（抜粋）を起草時の版に当てたもの（`rev_414AC0000000078_draft_r2_62`）に、後から公布され先に施行された
令3-37 の差分（`unit_derived_…`）を当てる。同じ id を改める第124条第3項が衝突として残る（空振り = 黙って壊れない） -/
theorem r2_62_then_r3_37_conflicts_only_at_124 :
    (applyUnit rev_414AC0000000078_draft_r2_62 unit_derived_414AC0000000078_20220401).map
      (fun r => r.conflicts.map (·.1)) =
      some ["414AC0000000078/main/chap:4/sec:1/subsec:2/art:124/para:3"] := by
  native_decide

/-- 加えた第178条・第180条の本文（参照は id、`bodies_…`）を令3-37 の後で描き直すと、変わるのは 2 項で、
その描画は e-Gov の 2022-04-01 版（令3-37 附則第63条が手当てした後）の第178条・第180条第4項の本文と一致する。
= 参照を id で持てば、附則第63条の手当ては生成できる（Rust `body::changed_refs` と同じ） -/
theorem r2_62_bodies_render_like_egov_after_r3_37 :
    (applyUnit rev_414AC0000000078_draft_r2_62 unit_derived_414AC0000000078_20220401).map (fun r' =>
      (haneFixes rev_414AC0000000078_draft_r2_62 r' bodies_502AC0000000062_art2).map (fun (id, _, after) => (id, after))) =
      some [("502AC0000000062/art2/art:178/new:1",
              ((rev_414AC0000000078_20220401.nodes.filter (·.art = "178")).map (·.text)).getD 0 ""),
            ("502AC0000000062/art2/art:180/new:4",
              ((rev_414AC0000000078_20220401.nodes.filter (·.art = "180")).map (·.text)).getD 3 "")] := by
  native_decide
