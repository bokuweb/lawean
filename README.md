...
| [docs/14-apply-benchmark.md](docs/14-apply-benchmark.md) | **当てる側の精度**: 衆議院の改め文（2017 年以降 3,620 単位）を e-Gov の改正前の版に当て、改正後の版と突き合わせる（`bench_apply`、`tools/fetch_egov_revisions.py`）。本則・附則・目次・項の中の表・別表を比べて一致 68.1%（段階施行は版ごとに文を確かめる）、identity patch の経路も一致の 97.8% で揃う。不一致は読み違い・当て違いの検出 | 実装 |
...
| [lawean-consolidate](crates/lawean-consolidate) | **他のプロジェクトから使う入口**（独立クレート、Rust だけ）。`consolidate(base_xml, 改め文) → 改正後の <Law> XML`、`verify(Input) → Report`、`consolidate_and_verify`。CLI `lawean-consolidate 発射台.xml 改め文.txt --out 改正後.xml [--expected …] [--enforced …]`。Lean は `lean` feature で任意 |
...
16. ~~検証ケースと playground~~（[docs/12](docs/12-cases.md)）: 実際の改正 24 件が通り、失敗例 15 件が指定した検査だけで落ちる。`lawean-check` + WASM playground
