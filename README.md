...
| [lawean-amend](crates/lawean-amend) | **改正**（[docs/08](docs/08-amendment.md)）。改め文パーサ（閉じた語彙 36 種。条ずれ・章節の挿入と繰り下げ・見出しと節名・項と文の差し替えを含む）、Source IR への apply、発射台・順序・ハネ（項と条）・連番の検査。実際の改正 22 件（13 法令。令和7年法律第47号の区分所有法・建替え円滑化法・被災マンション法・管理適正化法の 4 法令分を含む）で e-Gov の改正後リビジョンと一致。`ident`: 番号ベースの `Op` を発射台の stable_id に束縛して identity patch にする（繰り下げは消え、字句置換は本文全体の `replace` に、全部改正は `delete` + `insertAfter`）。Lean の `Ident.applyUnit` の写しで前検査 |
