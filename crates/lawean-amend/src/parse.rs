...
pub fn parse_instruction(line: &str) -> Result<Vec<Op>, ParseError> {
    ...
    let rules = RULES.get_or_init(|| {
        [
            ...
            r"^(?P<loc>.+?)中「(?P<a>.+?)」の下に「(?P<b>.+?)」を加える$",
            r"^(?P<loc>.+?)中第(?P<p>{N})項を第(?P<q>{N})項とし$",
            r"^第(?P<p>{N})項から第(?P<q>{N})項までを(?P<k>{N})項ずつ繰り(?P<dir>下げ|上げ)(?:る)?$",
            r"^(?P<loc>同項|.+?第{N}項)を同条第(?P<q>{N})項とし$",
            r"^(?P<loc>.+?)の次に次の一項を加える$",
            r"^(?P<loc>.+?)に次の一項を加える$",
            r"^第(?P<ch>{N})章に次の一条を加える$",
            r"^(?P<loc>.+?)を次のように改める$",
            r"^(?P<loc>.+?)を削る$",
        ]
        .iter()
