...
pub fn generate(fixtures: &std::path::Path) -> Vec<(String, String)> {
    ...
    // 起草から 3 年後に施行された改め文（docs/13 §2.4）: 令3-44 第7条（宅建業法、公布 2021-05-26、施行 2024-05-25）を
    // 起草時の版（2021-05-19）に束縛し、その間の 5 つの先行改正を 2021-05-19 版と 2024-04-01 版の差から起こす
    let t0 = rev("327AC1000000176_20210519_503AC0000000037");
    let t1 = rev("327AC1000000176_20240401_505AC0000000079");
    let t2 = rev("327AC1000000176_20240525_503AC0000000044");
    let u344 = parse_units(&read("amendments/503AC0000000044_art7.txt"))
        .unwrap()
        .remove(0);
    let b344 = bind(&t0, &u344, "503AC0000000044/art7").unwrap();
    let derived_t = lawean_amend::ident::derive_unit(
        &from_document(&t0),
        &from_document(&t1),
        "intervening/derived",
    );
    ...
    let revs = [
        ...
        ("Rev_327AC1000000176_20210519", "rev_327AC1000000176_20210519", "宅地建物取引業法 `327AC1000000176_20210519_503AC0000000037`（令和3年法律第44号 第7条の起草時の発射台）", from_document(&t0)),
        ("Rev_327AC1000000176_20240525", "rev_327AC1000000176_20240525", "宅地建物取引業法 `327AC1000000176_20240525_503AC0000000044`（令3-44 第7条の施行後。間に令3-37・令2-8・令4-61・令4-68・令5-79 が施行されている）", from_document(&t2)),
    ...
    let units = [
        ...
        ("Unit_503AC0000000044_art7", "unit_503AC0000000044_art7", "第11次地方分権一括法（令和3年法律第44号）第7条（宅地建物取引業法、`fixtures/amendments/503AC0000000044_art7.txt`）を起草時の版 `rev_327AC1000000176_20210519` に束縛したもの。第78条の3の全部改正と第78条の4の字句改め", b344.ops),
        ("Unit_derived_327AC1000000176_20240401", "unit_derived_327AC1000000176_20240401", "起草から施行までの 3 年に施行された 5 つの先行改正を、改め文ではなく 2021-05-19 版と 2024-04-01 版の差から起こした id の操作列（`ident::derive_unit`）", derived_t),
