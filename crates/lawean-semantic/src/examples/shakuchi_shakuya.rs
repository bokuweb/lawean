...
fn art04() -> Vec<Rule> {
    ...
    vec![
        ...
        rule("R4-1'")
            ...
            // 最初の更新では、10 年の原則（R4-1）だけでなく、その 10 年に対するただし書き（R4-2）も退く。
            // 当初は R4-1 だけを上書きしていたが、最初の更新で 11〜19 年と定めた世界で R4-1'（20 年）と
            // R4-2（その期間）が同時に適用され、更新後の期間に 2 つの値が要求される矛盾を、
            // Z3 の無矛盾検査（`lawean-verify::conflicts`）が見つけた。3 件目のバグ（docs/07）
            .overrides(&["R4-1", "R4-2"])
            .provenance(&p(1), Confidence::High, BY)
