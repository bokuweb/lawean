...
**時間表現 → Semantic IR の値・条件**（`skeleton::to_model`、2026-09-22）: 「借地権の存続期間は、三十年とする」→ `Set{存続期間, Duration(30y)}`、
「その期間は、更新の日から十年とする」→ `Set{期間, Period(更新の日 + 10y)}`、条件節の「存続期間を五十年以上として」→ `Cmp(var(存続期間), Ge, 50y)`、
「〜の日から六月を経過した後」→ `Time(Elapsed)`。残りの条件文は `Unknown` として `And` に残す（落とさない）。
手書き IR（借地借家法 8 条）との突き合わせ（`tests/values.rs`）: Set の期間・時点 2/3、条件の比較 1/2。外れたのは括弧書きの場合分け「（最初の更新にあっては、二十年）」と照応「これより長い期間」で、どちらも層 2。
