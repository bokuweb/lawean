...
## 何を「証明」と呼ぶか（2026-09-21）

- **改正の検証**（溶け込み・参照・順序・衝突）は文字列と id の決定的な操作についての命題で、証明できる。「全法令・全改正について可換」「この改正を当てると e-Gov の版になる」。実際に起きた失敗（公職選挙法 平成30年法律第75号）もこの種類。ここが本線
- **内容の検査**（Semantic IR の性質）は「法令が正しい」ことの証明ではない。法令に外部の仕様は無い。証明できるのは、内部の無矛盾、**人が書いた性質**を形式化した法令が満たすこと（起案者の意図と条文の食い違いの検査）、改正の前後で性質が保たれること（影響分析）。形式化自体が解釈である（[ADR-0005](adr/0005-no-single-interpretation.md)）ので、結果は検討資料であり、「法律が正しいと証明した」とは言わない
